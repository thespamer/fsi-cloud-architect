...
locals {
  ...
  # MTU. 8896 is jumbo and is unavailable when the attachment is encrypted.
  # Remember the end-to-end path minimum: AWS transit VIF caps at 8500.
  attachment_mtu = 8896
}
...
# AWS: 10, 100, 400 Gbps. Order in PAIRS across edge availability domains, and
# repeat in a second metro for the 99.99% tier.
#
...
variable "noc_email" { type = string }
variable "peer_ip_ead1" { type = string }
variable "peer_ip_ead2" { type = string }
variable "aws_remote_location" { type = string }
